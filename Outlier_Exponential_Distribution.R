...
gesd_res <- gesdTest(Population, maxr = 1000)
...
kmed.outliers <- head(kmedoids_out, 20) # represent the first 20 large distance seen to be outliers after plotting. You can increase it and see the resulting plot.
...
fit.exp_clean <- fitdistr(clean, "exponential")
clean_rate <-  fit.exp_clean$estimate #  exponential parameter "rate" from the "clean" data
fit.exp_contaminated <- fitdistr(contaminated, "exponential")
contaminated_rate <- fit.exp_contaminated$estimate #  exponential parameter "rate" from the "clean" data

                                                    
new_data_a <- rexp(n = 2500, rate = clean_rate) # generating a exponential distribution based on the "clean" data
new_data_b <- rexp(n = 1000, rate = contaminated_rate ) # generating a exponential distribution based on the "contaminated" data
...
outlier_simulation <- function(N = N, n = n, outlier_n = outlier_n,
                               dist = dist, Z_score_threshold = 
                                 Z_score_threshold, GESD_p_value = GESD_p_value,
                               ...
                               maxr = maxr, k_medoids_outliers = k_medoids_outliers, som_maps_outliers = som_maps_outliers) 
{
  ...
  knearest.out <- nn(as.matrix(Sample),cutoff = 0.95, Method = "euclidean", # Applying K-Nearest Neighbor
                     rnames = FALSE, k = 3, boottimes = 1000)
  knn_out <- knearest.out$`Outlier Observations` # Those are the outliers detected by the method
  ...
  kmedoids_outliers <- Sample[kmed.outliers] # Those are the outliers detected by the method
  ...
  som_outliers <- Sample[som_out] # Those are the outliers detected by the method
  ...
  fit.exp_clean <- fitdistr(clean, "exponential") # Fitting the clean data to an exponential distribution to estimate the parameter
  clean_rate <-  fit.exp_clean$estimate # Saving the "rate" parameter of the exponential from the "clean" and "contaminated"
  fit.exp_contaminated <- fitdistr(contaminated, "exponential") #Fitting the contaminated data to an exponential distribution to estimate the paramete
  contaminated_rate <- fit.exp_contaminated$estimate # Dataset. This wil be needed for the generation of the two exponential distributed data sets
                                                    
  
  new_data_a <- rexp(n = new_data_a_n, rate = clean_rate) # Creating an exponential distribution based on the "clean" data
  new_data_b <- rexp(n = new_data_b_n, rate = contaminated_rate) # Creating a poisson distribution based on the "contaminated" data
  ...
  Zscore_outliers <- new_data_a_b[which(scale(new_data_a_b) > 3),] # Those are the outliers detected by the method
  
  gesd_res <- gesdTest(as.matrix(new_data_a_b), maxr = maxr) # Applying the GESD
  outliers.GESD <- new_data_a_b[which(gesd_res$p.value < 0.05),]  
  
  new_data_a_b <- as.matrix(new_data_a_b) # Applying the IQR 
  IQR.out <- IQR.outliers(new_data_a_b) # Those are the outliers detected by the method
  
  knearest.out <- nn(as.matrix(new_data_a_b), k= 3, cutoff = 0.95, 
                     Method = "euclidean", rnames = FALSE, boottimes = 1000)  #Applying the K-nearest neighbor
  knn_out <- knearest.out$`Outlier Observations` # Those are the outliers detected by the method
  ...
  kmedoids_out <- order(kmed.distance, decreasing=T)
  kmed.outliers <- head(kmedoids_out, k_medoids_outliers)
  kmedoids_outliers <- new_data_a_b[,1][kmed.outliers] # Those are the outliers detected by the method
  ...
  som_distance <- order(som_model$distances, decreasing = T)
  som_out <-head(som_distance, som_maps_outliers)
  som_outliers <- new_data_a_b[,1][som_out]    # Those are the outliers detected by the method
...
outlier_n <- 50
outlier_simulation(N = 500000, n = 1000, outlier_n = outlier_n, Z_score_threshold = 3, GESD_p_value = 0.05,
                   new_data_a_n = n, new_data_b_n = 2000, maxr = 5, 
...
for(i in 1:100){ # Number of Simulation runs
  resultX <- outlier_simulation(N = 50000, # Number of elements in synthetic population
                                n = 1000,   # Sample size (SRS)
                                outlier_n = 100, # Number of outliers sampled from contaminated data set
...
Res_Expo_6 <- c((True_Zscore_result <- mean(unlist(lapply(result, "[", 1)))),
...
Res_Expo_1  # (1000, 0.01) number of outlier = 10
Res_Expo_2 # (1000, 0.025) number of outlier = 25
Res_Expo_3 # (1000, 0.05)  number of outlier = 50

Res_Expo_4  # (10000, 0.01) number of outliers = 100
Res_Expo_5 # (10000, 0.025) number of outliers = 250
Res_Expo_6 # (10000, 0.05)  number of outliers = 500

Res_Expo_7  # (100000, 0.01) number of outliers = 1000
Res_Expo_8 # (100000, 0.025) number of outliers = 2500
Res_Expo_9 # (100000, 0.05)  number of outliers = 5000


simulation.results.exp <- rbind(Res_Expo_1,Res_Expo_2,Res_Expo_3,Res_Expo_4,Res_Expo_5,Res_Expo_6)




setwd("C:/Users/mathi/Dropbox/Universität/3rd Semester/Research Project/Results")
write.csv(Result_1,"C:/Users/mathi/Dropbox/Universität/3rd Semester/Research Project/Results/simulation_results_outliers.csv", row.names = T)
simulation_results <- read.csv("Simulation_results_outliers.csv", header = TRUE)

# Converting Dataset size and Percentage of outliers to factor.

simulation_results$Percentage.of.outliers <- as.factor(simulation_results$Percentage.of.outliers)
simulation_results$Size.Dataset <- as.factor(simulation_results$Size.Dataset)
...
simulation_results$False.Alarm.Rate <- gsub(",", "", simulation_results$False.Alarm.Rate)
simulation_results$False.Alarm.Rate <- as.numeric(simulation_results$False.Alarm.Rate)
simulation_results$False.Alarm.Rate <- simulation_results$False.Alarm.Rate / 100

str(simulation_results)

par(mfrow=c(1,3))

figure_1 <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 100),],
                   aes(x = Percentage.of.outliers, y = Detection.Rate, 
                       col = Method,
                       group = Method)) + 
  geom_point() +
  geom_line() +
  ...
  theme_classic()
figure_1
figure_2 <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 500),],
                   aes(x = Percentage.of.outliers, y = Detection.Rate, 
                       col = Method,
                       group = Method)) + 
  geom_point() +
  ...
  theme_classic()
figure_2
figure_3 <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 1000),],
                   aes(x = Percentage.of.outliers, y = Detection.Rate, 
                       col = Method,
                       group = Method)) + 
  geom_point() +
...
figure_4 <- figure_2 <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 100),],
                               aes(x = Percentage.of.outliers, y = False.Alarm.Rate, 
                                   col = Method,
                                   group = Method)) + 
  geom_point() +
  geom_line() +
  ...
  theme_classic()
figure_4
figure_5 <- figure_2 <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 500),],
                               aes(x = Percentage.of.outliers, y = False.Alarm.Rate, 
                                   col = Method,
                                   group = Method)) + 
  geom_point() +
  ...
  theme_classic()
figure_5
figure_6  <- ggplot(data = simulation_results[which(simulation_results$Size.Dataset == 1000),],
                    aes(x = Percentage.of.outliers, y = False.Alarm.Rate, 
                        col = Method,
                        group = Method)) + 
  geom_point() +
  ...
  theme_classic()
figure_6
grid.arrange(figure_4, figure_5, figure_6, ncol=3)
